...
| **剧本** | 导入或直接编写剧本原文 |
| **流程图** | 拆解之后自动画出这个项目的**真实依赖图**。分镜与视频同一行直连；镜头按场次、资产按类别分组；悬停/单击某个节点只点亮与它相关的连线，多对多的引用关系一眼看清；双击跳到对应条目 |
...
- **流程图重新排版**：分镜与视频一一对应放在同一行（连线是水平直线）；镜头按场次、
  资产按类别分组并带组标题；剧本/拆解/成片垂直居中。连线默认压暗，**悬停或单击某个
  节点只点亮与它相关的线**——想知道某个角色被哪几个镜头引用，把鼠标放上去就看到。
  双击跳到对应条目，点空白取消选中。
- 概览顶部新增**流水线带**：四段相连的进度条，一眼看到每个阶段的完成度与审核状态。
- 控制台每行带级别图标（· / ! / ✖），文本可选中复制。

## 0.4.0 有什么变化

- **默认逐条生成，批量要自己找出来**。审核通过不会触发任何生成；工作台的主按钮是
  「生成所选」，卡片可勾选，每个分组标题旁有「生成本组缺失」，右侧检查器有「生成这一条」。
  「生成全部缺失 / 全部重生成」收进了「批量…」菜单。
- **可以上传自己的素材**：检查器里「上传图片…／上传片段…」，图片会自动转成 PNG 放到
  该条目该在的位置。上传的条目标记为「自备」，**批量生成不会覆盖它**，只有单独点
  「生成这一条」才会重做。
- **资产按类别分组显示**：角色 / 服装 / 道具 / 场景 各成一组；分镜与视频按场次分组。
- **xAI 支持视频**（按 OpenAI 兼容的「提交任务 → 轮询 → 取回」形态调用，容忍多种字段命名）；
  OpenAI 在视频这一格移除。
- 界面细节：导航用左侧色条标识当前页，字段标签右对齐，分组标题带色条，
  工具栏不再是一排按钮。

## 0.3.4 有什么变化

- **看得见在跑**：拆解等长任务会实时显示接收进度——顶栏和当前页面的状态条上有
  「接收中 1234 字 · 已用 18 秒」，控制台每 5 秒留一条记录，页面上就能直接取消。
  推理模型只吐思维链时也会显示「模型思考中」，不再像是卡死。
- **兼容更多中转的返回格式**：除标准 `delta.content` 外，也接受整条 `message.content`
  与老式 `text`；`reasoning_content` 识别为思考、不计入正文。
- 收到了流式分片却没有正文时，会明说「该中转字段可能与 OpenAI 不一致」并附原文，
  而不是一句空洞的失败。
