...
| **剧本** | 导入或直接编写剧本原文；**AI 格式化**可把随意文本整理成标准影视剧本模板（原稿自动备份 .bak） |
...
| **资产 / 分镜 / 视频** | 条目工作台：资产按**角色/服装/道具/场景分页**；分镜每镜 **N 帧关键帧**（首帧…末帧，全局可设、按镜头可覆盖），末帧衔接下一镜首帧、视频用首末双帧约束，片段之间接得上；右侧检查器看图、改 prompt、逐条生成、上传自备素材 |
| **配音与字幕** | 逐镜头台词配音（云端 TTS 或**本地 Piper**，离线免额度）、SRT 字幕生成、成片时可选混音与烧录；**本地工具**（ffmpeg / Piper / 中文音色）一键从上游官方发布下载最新版 |
| **设置** | **按能力配置且互相独立**：对话 / 生图 / 视频 / 配音四张卡——选供应商 → 端点 → 密钥 → 从拉取到的模型里选 |
...
- **配音**：新增「配音」能力（第四张模型卡）。逐镜头把台词转语音——云端走 OpenAI 兼容
  TTS，或用**本地 Piper 引擎**（离线、免额度，引擎与中文音色一键从上游下载）。
  可上传自己的配音；成片时可选把配音混入音轨。
- **字幕**：按镜头时长自动生成 SRT；成片时可选烧录进画面。
- **ffmpeg 变成托管依赖**：「配音与字幕 → 本地工具」一键从 BtbN 官方自动构建下载
  最新版（Windows zip / Linux tar.xz），装进本机配置目录；已有系统安装则直接用。
  CLI：`adrama tools --install ffmpeg`。
- **分镜多帧**：每镜生成 N 张关键帧（默认 2：首帧+末帧；全局 2–8 可调，
  分镜检查器里可按镜头单独覆盖）。按顺序链式生成——本镜第 i 帧参考第 i-1 帧、
  **下一镜首帧参考上一镜末帧**；视频用首末双帧约束（Veo lastFrame），
  片段之间自然衔接。
- **AI 格式化剧本**：剧本页一键把随意文本整理成标准影视剧本模板
  （场次行 / 内外景 / △动作 / 台词 / OS·VO /【字幕·空镜】），原稿自动备份 .bak。
- **项目迁移**：概览「打包迁移…」把整个项目打成 `.adrama.tar.gz`，
  另一台设备「导入项目包…」直接继续做（密钥按设计不随包走）。
  CLI：`adrama pack` / `adrama unpack`。
- **资产分页**：角色 / 服装 / 道具 / 场景分页签展示，不再一页堆到底。
- **对比度整体调亮**：正文、次要文字与边框都提了一档，不再深底配深字。
